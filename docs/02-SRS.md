...
| ID | FR-031 |
| Priority | Must |
| Description | `applications` is a **per-instance G-counter** (`BTreeMap<InstanceId, u32>`); the effective count `n` (for `F(n)`) and the public `applications()` accessor return the **sum** of values — no consumer breakage. `reinforce_experience()` keeps its signature; it atomically increments **the local instance's key** and sets `last_reinforced = now`. Explicit-only — no read/recall path mutates energy state; reads stay out of the sync WAL. **Sync merge (D7):** per-key max over the map (commutative + idempotent) ⇒ **exact total**; `last_reinforced` merges `max` (latest-wins). Convergent *and* exact — concurrent reinforcements on different instances are summed, not lost. **Precondition (identity hygiene, VS-3.5.1 slice-close):** "exact total" holds **only when each replica carries a distinct `InstanceId`** — the G-counter's actor-uniqueness invariant. `InstanceId` is persisted in the DB file, so a *clone/snapshot/restore* of a store yields replicas with a duplicate id whose buckets collide under per-key max (increments silently lost). **Identity remint (r1.s1.w2, closes #10):** `PulseDB::instance_id()` exposes the persisted id and `PulseDB::remint_instance_id()` replaces it with a fresh UUID in one write transaction (cached value updated in step; old `applications` buckets left untouched so totals are preserved; no WAL/sync event; a `tracing::info!` carries old and new ids; read-only stores return `ReadOnly`). **Rule: remint a restored copy before its first reinforce**, and before constructing a `SyncManager` over it — the manager reads the identity once at construction, and behaviour with a live manager is unspecified (no runtime guard; explicit API only, no heuristic clone detection — triage D5). Likewise `last_reinforced`'s `max` merge trusts peer wall-clocks (no skew bound — issue #13). |
...
#### NFR-020: Storage-Format Upgrade Safety
